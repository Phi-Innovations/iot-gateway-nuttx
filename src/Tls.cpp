...
void Tls::test(void) {
    ...
    const selftest_t selftests[] = {
        ...
        {"entropy", Tls::mbedtls_entropy_self_test_wrapper},
#endif
...
#if defined(MBEDTLS_MEMORY_BUFFER_ALLOC_C)
int Tls::mbedtls_memory_buffer_alloc_free_and_self_test( int verbose )
{
    if( verbose != 0 )
    {
...
int Tls::calloc_self_test( int verbose )
{
    int failures = 0;
...
int Tls::test_snprintf( size_t n, const char *ref_buf, int ref_ret )
{
    int ret;
...
int Tls::run_test_snprintf( void )
{
...
int Tls::mbedtls_entropy_self_test_wrapper( int verbose )
{
#if defined(MBEDTLS_ENTROPY_NV_SEED) && !defined(MBEDTLS_NO_PLATFORM_ENTROPY)
